 ...
 * Adapted from Fortran 77 PCCTS grammar
...
functionStatement :
	(type)?
	FUNCTION^ s: NAME LPAREN! (namelist)? RPAREN! seos ;
...
varRef :
	(s:NAME^ {variables.add(s.getText());} | n:"real"^ {n.setType(NAME);}) (subscripts ( substringApp )? )? ;
...
identifier { Token id = LT(1); }:
	s:NAME {variables.add(s.getText());}|
	( "real" )
	{ id.setType(NAME); }
	;
