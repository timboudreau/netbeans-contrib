...
public final class FolderList extends javax.swing.JPanel {
    ...
    public void setComponentsEnabled(boolean enabled){
        ...
        if (enabled) removeButton.setEnabled(roots.getSelectedIndices().length != 0);
            else removeButton.setEnabled(enabled);
    }
    ...
    private void initComponents() {
        ...
        removeButton.setText(bundle.getString("CTL_RemoveFolder")); // NOI18N
    ...
    static boolean isValidRoot (File file, File[] relatedRoots, File projectFolder) {
        Project p;
        if (file!=null && (p = FileOwnerQuery.getOwner(file.toURI()))!=null 
            && (projectFolder==null || !file.getAbsolutePath().startsWith(projectFolder.getAbsolutePath()+File.separatorChar))) {
