...
class ErlangCodeCompletion extends CodeCompletionHandler {
   ...
   * kick in.
   */
  override def getPrefix(pResult:ParserResult, lexOffset:Int, upToOffset:Boolean) :String = {
    try {
      val doc = LexUtil.document(pResult, true) match {
  ...
  override def resolveTemplateVariable(variable:String, info:ParserResult, caretOffset:Int, name:String, parameters:Map[_, _]) :String = {
  ...
  override def getApplicableTemplates(info:ParserResult, selectionBegin:Int, selectionEnd:Int) :Set[String] = {
   ...
   */
  override def resolveLink(_link:String, elementHandle:ElementHandle) :ElementHandle = {
    var link = _link
    if (link.indexOf(':') != -1) {
      link = link.replace(':', '.')
