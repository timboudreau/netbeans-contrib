...
class ErlangStructureAnalyzer extends StructureScanner {
  ...
  override def scan(result:ParserResult) :List[StructureItem] = result match {
  ...
  override def folds(result:ParserResult) :Map[String, List[OffsetRange]] = result match {
    case pResult:ErlangParserResult =>
  ...
  private class ErlangStructureItem(val dfn:AstDfn, pResult:ParserResult) extends StructureItem {
    ...
    override def getModifiers :Set[Modifier] = dfn.getModifiers
    ...
    override def getNestedItems : List[StructureItem] = {
