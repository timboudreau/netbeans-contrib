...
class ErlangFormatter extends Formatter {
  ...
  override def needsParserResult = false
  ...
  override def reformat(context:Context,  pResult:ParserResult) :Unit = {
    reindent(context, context.document, context.startOffset, context.endOffset, pResult, false)
  }

  override def indentSize = 2//codeStyle.getIndentSize

  override def hangingIndentSize = 0//codeStyle.getContinuationIndentSize();
  ...
  private case class Brace(var token:Token[TokenId],
    ...
    override def toString = token.text.toString
  }
