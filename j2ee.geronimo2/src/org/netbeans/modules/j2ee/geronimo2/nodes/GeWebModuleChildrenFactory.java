...
import org.netbeans.modules.j2ee.geronimo2.GeTargetModuleID;
import org.openide.nodes.ChildFactory;
import org.openide.nodes.Node;
import org.openide.util.Lookup;

/**
 * Uses the weird lookups
 * @author Max Sauer
 */
public class GeWebModuleChildrenFactory extends ChildFactory<GeWebModule> {

    private static final GeWebModule MODULE_WAITING_MARK = new GeWebModule(null, null, false);
    private final Lookup lookup;

    public GeWebModuleChildrenFactory(Lookup lookup) {
        this.lookup = lookup;
    }
     ...
     */
    @Override
    protected Node createNodeForKey(GeWebModule key) {
        if (key == MODULE_WAITING_MARK) {
            return createWaitNode();
        }

        GeWebModuleNode node = new GeWebModuleNode(key);
        key.setRepresentedNode(node);
        return node;
    }

    @Override
    protected boolean createKeys(List<GeWebModule> toPopulate) {
        ...
        if (manager instanceof GeDeploymentManager && target != null) {
            ...
            try {
                ...
                for (int i = 0; i < modules.length; i++) {
                    ...
                    toPopulate.add(new GeWebModule(manager, targetModuleID, true));
                }

                modules = manager.getNonRunningModules(ModuleType.WAR, new Target[] {target});
                for (int i = 0; i < modules.length; i++) {
                    TargetModuleID targetModuleID = (TargetModuleID) modules[i];
                    toPopulate.add(new GeWebModule(manager, targetModuleID, false));
                }
        ...
        Collections.sort(toPopulate, GeWebModule.GE_WEB_MODULE_COMPARATOR);
        return true;
    }
