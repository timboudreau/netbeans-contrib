...
/**
 * Super node for all application types
 * @author Max Sauer
 */
public class AppNode extends AbstractNode {

    public AppNode(Lookup lookup) {
        super(new ApplicationsChildren(lookup));
        setDisplayName(NbBundle.getMessage(AppNode.class, "LBL_Apps"));
    }
    ...
    private static class ApplicationsChildren extends Children.Keys<Node> {
        ...
        public ApplicationsChildren(Lookup lookup) {
            setKeys(new Node[]{
                ...
                GeEjbModuleNode.newInstance(lookup),
                GeWebModuleNode.newInstance(lookup)
            });
        }
