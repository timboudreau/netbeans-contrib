...
public class HintsTask {
    ...
    public HintsTask(Map<Kind, List<TreeRule>> hints) {
        this.hints = hints;
    }
    
    public List<? extends ErrorDescription> compute(CompilationInfo info) throws Exception {
        Map<Kind, List<TreeRule>> hints = this.hints == null ? RulesManager.getInstance().getHints() : this.hints;
        ...
        new ScannerImpl(info, new AtomicBoolean(), hints).scan(info.getCompilationUnit(), result);
