 ...
 */
public class PradoCssModel extends CssModel {

    private static final Logger LOGGER = Logger.getLogger(PradoCssModel.class.getName());
    private static final boolean LOG = LOGGER.isLoggable(Level.FINE);

    public static PradoCssModel get(Document doc) {
        PradoCssModel model = (PradoCssModel) doc.getProperty(PradoCssModel.class);
        if (model == null) {
            model = new PradoCssModel(doc);
            doc.putProperty(PradoCssModel.class, model);
        }

        return model;
    }

    private PradoCssModel(Document doc) {
        super(doc);
    }
    ...
    private void extractCssFromPage(Document doc, StringBuilder buffer) {
        HashMap<String, Object> state = new HashMap<String, Object>(6);
        TokenHierarchy<?> th = TokenHierarchy.get(doc);
        TokenSequence<PageTokenId> ts = th.tokenSequence(PageTokenId.language());
        if(ts == null) {
        ...
        ts.moveStart();
        while (ts.moveNext()) {
            Token<PageTokenId> token = ts.token();
            if (token.id() == PageTokenId.T_INLINE_HTML) {
