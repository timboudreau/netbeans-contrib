...
public class HighlightSections {
    ...
    private static final AttributeSet ltColors =
            AttributesUtilities.createImmutable(StyleConstants.Background,
            new Color(235, 0, 0));
...
//    }
    ...
    public HighlightSections(Document doc) {
...
//    }
    ...
    public void setSelectedNode(Document selectedDocument, TreeCreator.TreeASTNodeAdapter selectedNode) {
        ...
        if(selectedNode != null) {
            bag.addHighlight(selectedNode.start, selectedNode.end, defaultColors);
        }
        this.selectedNode = selectedNode;
    }
    
    public void addLocationToken(Node node) {
        LocationTokenVisitor visitor = new LocationTokenVisitor();
        node.accept(visitor);
    }
    ...
    private class LocationTokenVisitor extends DefaultScanner {

        @Override
        public void scan(Node node) {
//            System.out.println("Added: " + node.getClass().getSimpleName() + " [" + node.position() + ", " + (node.position() + node.length()) + "[");
            if (node.position() > 0)
                bag.addHighlight(node.position(), node.position() + node.length(), ltColors);
        }
