...
import org.netbeans.modules.fortress.editing.rats.LexerFortress;
 ...
 * @author Caoyuan Deng
 */
public class FortressLexer implements Lexer<FortressTokenId> {
    ...
    public Token<FortressTokenId> nextToken() {
        Result result = scanOneToken();
        if (result != null) {
            FortressTokenId tokenId = (FortressTokenId) result.semanticValue();
            /**
            ...
            if (input.readLength() == 0) {
                ...
                Token<FortressTokenId> token = tokenFactory.createToken(tokenId);
                return token;
            }
        } else {
            return null;
    ...
    private Result scanOneToken() {
         ...
         */
        LexerFortress lexer = new LexerFortress(lexerInputReader, "<current>");
