...
public class AstUtilities {
    ...
    public static Node getRoot(CompilationInfo info) {
        return getRoot(info, FortressMimeResolver.MIME_TYPE);
    }

    public static FortressParserResult getParserResult(CompilationInfo info) {
        ParserResult result = info.getEmbeddedResult(FortressMimeResolver.MIME_TYPE, 0);

        if (result == null) {
            return null;
        } else {
            return (FortressParserResult) result;
        }
    }
    ...
    public static Node getRoot(ParserResult r) {
        assert r instanceof FortressParserResult;

        FortressParserResult result = (FortressParserResult) r;
