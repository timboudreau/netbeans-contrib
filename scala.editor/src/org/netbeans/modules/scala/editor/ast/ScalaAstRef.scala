 ...
 * @author Caoyuan Deng
 */
class ScalaAstRef(_idToken:Option[Token[TokenId]], _kind:ElementKind) extends AstRef(_idToken, _kind) {
  import ElementKind._

  def getMimeType :String = ScalaMimeResolver.MIME_TYPE

  override def isOccurrence(ref:AstRef) :Boolean = ref.getKind match {
