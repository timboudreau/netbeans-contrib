...
import org.netbeans.modules.scala.editor.ast._
...
abstract class AstVisitor(rootNode:Node, th:TokenHierarchy[_]) extends Visitor {
  ...
  override def visit(a:Annotation) : Object = {
  ...
  protected def boundsTokens(node:Node) :Array[Token[_ <: TokenId]] = {
    val loc = node.getLocation
    val ts = ScalaLexUtil.tokenSequence(th, loc.offset).get
    ...
    var startToken = ScalaLexUtil.findNextNonWs(ts)
    ...
    var endToken = ScalaLexUtil.findPreviousNonWs(ts)
   ...
   */
  protected def idToken(idNode:Node) :Option[Token[_ <: TokenId]] = {
    ...
    val token = idNode.getName match {
      case "VarId"     => ScalaLexUtil.findNext(ts, ScalaTokenId.Var)
      case "RecId"     => ScalaLexUtil.findNext(ts, ScalaTokenId.Rec)
      case "AtomId"    => ScalaLexUtil.findNext(ts, ScalaTokenId.Atom)
      case "MacroId"   => ScalaLexUtil.findNext(ts, ScalaTokenId.Macro)
      case "PredAttr"  => ScalaLexUtil.findNext(ts, ScalaTokenId.Atom)
      case "Attribute" => ScalaLexUtil.findNext(ts, ScalaTokenId.Atom)
    }
