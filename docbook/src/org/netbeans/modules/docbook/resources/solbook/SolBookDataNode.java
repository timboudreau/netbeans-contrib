...
class SolBookDataNode extends DataNode {
    private final InstanceContent content;
    public SolBookDataNode(SolBookDataObject obj, InstanceContent content) {
        super(obj, Children.LEAF, new AbstractLookup (content));
        OpenCookie oc = obj.getCookie(OpenCookie.class);
        assert oc != null : obj + " has no OpenCookie; DBES=" + obj.getCookie(SolBookEditorSupport.class);
        ...
        setIconBaseWithExtension("org/netbeans/modules/docbook/resources/solbook/templates/solbook.png");
    }
...
/*        if (mime.equals(SolBookDataLoader.MIME_DOCBOOK)) {
            return NbBundle.getMessage(SolBookDataNode.class, "HINT_file_docbook_xml"); //NOI18N
        } else if (mime.equals(SolBookDataLoader.MIME_SLIDES)) {
            return NbBundle.getMessage(SolBookDataNode.class, "HINT_file_slides"); //NOI18N
        } else */ if (mime.equals(SolBookDataLoader.MIME_SOLBOOK)) {
            return NbBundle.getMessage(SolBookDataNode.class, "HINT_file_solbook_xml"); //NOI18N
        } else {
    ...
    private static final class Notifier implements MainFileProvider.Notifier {
        private SolBookDataObject obj;
        Notifier (SolBookDataObject obj) {
            this.obj = obj;
        }

        public void change() {
            SolBookDataNode n =
                    (SolBookDataNode) obj.getNodeDelegate();
            n.change();
        }
    }
}
