...
import org.netbeans.modules.ide.analysis.modernize.impl.ModernizeAnalyzerImpl.ResponseImpl;
import org.netbeans.modules.ide.analysis.modernize.impl.YamlParser.Replacement;
import org.netbeans.modules.ide.analysis.modernize.options.AnalyzerPreferences;
import org.netbeans.modules.ide.analysis.modernize.options.ClangAnalyzerOptions;
import static org.netbeans.modules.ide.analysis.modernize.utils.AnalyticsTools.fatalError;
import static org.netbeans.modules.ide.analysis.modernize.utils.AnalyticsTools.findItem;
import java.io.File;
    ...
    public void postProcess(boolean isAnalyzer, CsmFile startFile, Lookup.Provider project, List<YamlParser.Diagnostics> results, CsmErrorProvider.Request request, CsmErrorProvider.Response response) {
        ...
        for (YamlParser.Diagnostics diag : results) {
            ...
            if (startFile.equals(file) && csmFile.equals(file)) {
                message = String.format("[%s]: %s", diag.getCheckName(), diag.getMessage()); //NOI18N
            } else {
                message = String.format("[%s; %s]: %s", diag.getCheckName(), startFile.getName(), diag.getMessage()); //NOI18N
            }

            ModernizeErrorInfo info = new ModernizeErrorInfo(diag, message, project);  //NOI18N
