...
public class PortletDDHelper {
                ...
                if(portletType != null && portletType.length > 0) {
                    ...
                    for(int ii=0; null != initParams && ii < initParams.length;ii++) {
                        ...
                        if(inMode.equals(PortletModeType.VIEW)) {
                            if(initParams[ii].getName().equals(PortletDDProperties.INIT_VIEW)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        } else if(inMode.equals(PortletModeType.EDIT)) {
                            if(initParams[ii].getName().equals(PortletDDProperties.INIT_EDIT)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        } else if(inMode.equals(PortletModeType.HELP)) {
                            if(initParams[ii].getName().equals(PortletDDProperties.INIT_HELP)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        }
                    ...
                    if(!foundMode) {
                         ...
                         */
                        InitParamType initParam = portletType[0].newInitParamType();
                        if(inMode.equals(PortletModeType.VIEW)) {
                            initParam.setName(PortletDDProperties.INIT_VIEW);
                        } else if(inMode.equals(PortletModeType.EDIT)) {
                            initParam.setName(PortletDDProperties.INIT_EDIT);
                        } else if(inMode.equals(PortletModeType.HELP)) {
                            initParam.setName(PortletDDProperties.INIT_HELP);
                        }
                   ...
                   SupportsType [] supportsTypes = portletType[0].getSupports();
                    ...
                    if(!foundPortletMode) {
                         ...
                         */
                        if(!foundSupports) {
                            supportsType = portletType[0].newSupportsType();
                        } else {
            ...
            if(null!=portletApp) {
                ...
                if(portletType != null && portletType.length > 0) {
                    ...
                    for(int ii=0; null != supportsTypes && null != modeToRemove && ii < supportsTypes.length;ii++) {
                        ...
                        if(modeToRemove.equals(PortletDDProperties.INIT_VIEW)) {
                            supportsTypes[ii].removePortletMode(PortletModeType.VIEW.toString());
                        } else if(modeToRemove.equals(PortletDDProperties.INIT_EDIT)) {
                            supportsTypes[ii].removePortletMode(PortletModeType.EDIT.toString());
                        } else if(modeToRemove.equals(PortletDDProperties.INIT_HELP)) {
            ...
            if(portletType != null && portletType.length > 0) {
                ...
                for(int ii=0; null != initParams && ii < initParams.length;ii++) {
                    ...
                    if(null != pageValue && pageValue.equals(inPageName)) {
                        ...
                        } else if (initParams[ii].getName().equals(PortletDDProperties.INIT_EDIT)) {
                            returnMode = PortletModeType.EDIT;
                        } else if (initParams[ii].getName().equals(PortletDDProperties.INIT_HELP)) {
  ...
  public static boolean isRubyPortletEntryPresent(PortletApp portletApp) {
        if (portletApp == null) {
            return false;
        }
        PortletType[] portletTypes = portletApp.getPortlet();
        for (PortletType portletType : portletTypes) {
            InitParamType[] intParams = portletType.getInitParam();
            for(InitParamType ip:intParams)
            {
                if(ip.getName().equals(RubyPortletConstants.GLOBAL_FILES)
                        || ip.getName().equals(RubyPortletConstants.ACTION_URI))
                    return true;
                
            }
//            if (portletType.getPortletClass().equals(RubyPortletConstants.PHP_PORTLET_CLASS)) {
//
//                return true;
//            }
        }
