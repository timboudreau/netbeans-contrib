        ...
        try {
            scanFinished = js.runWhenScanFinished(new CancellableTask<CompilationController>() {
                ...
                public void run(CompilationController ci) throws Exception {
                    ...
                    //TODO XXX if tree is CompilationUnitTree Excpetion will be thrown
/*                    
                    if (((JavaFXTree)tree).getJavaFXKind() == JavaFXTree.JavaFXKind.FUNCTION_DEFINITION){
                        ...
                        if (el != null && (el.getKind() == ElementKind.METHOD || el.getKind() == ElementKind.CONSTRUCTOR)) {
                            ...
                            if (te != null) {
                                currentMethodPtr[2] = getBinaryName(te);
                            }
                        }
                    }
 */
...
//                            JavafxcScope scope = ci.getTreeUtilities().javafxcScopeFor(offset);
    ...
    public static String getBinaryName (TypeElement element) throws IllegalArgumentException {
        if (element instanceof JavafxVarSymbol.TypeSymbol) {
            return ((JavafxVarSymbol.TypeSymbol)element).flatName().toString();
        }
        else {
