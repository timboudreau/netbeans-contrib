...
public class BreakpointsActionsProvider implements NodeActionsProviderFilter {
    ...
    private static final Action GO_TO_SOURCE_ACTION = Models.createAction (
        ...
        new Models.ActionPerformer () {
            ...
            public void perform (Object[] nodes) {
                goToSource ((JavaFXBreakpoint) nodes [0]);
            }
        },
    ...
    throws UnknownTypeException {
        if (!(node instanceof JavaFXBreakpoint) || node instanceof ThreadBreakpoint)
    ...
    public void performDefaultAction (NodeActionsProvider original, Object node) throws UnknownTypeException {
        if (node instanceof JavaFXBreakpoint && !(node instanceof ThreadBreakpoint)) 
            goToSource ((JavaFXBreakpoint) node);
        else
            original.performDefaultAction (node);
    }

    private static void goToSource (JavaFXBreakpoint b) {
        String url;
        int lineNumber;
