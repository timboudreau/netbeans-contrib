 ...
 * @author Satyaranjan
 */
public class ShowInStoryBoardAction extends CookieAction implements Runnable {
    ...
    public void run() {
      //   PortletDataObject dataObject = (PortletDataObject)currentNode.getLookup().lookup(PortletDataObject.class);
        //PortletDataObject dataObject = (PortletDataObject) node.getDataObject();
              IPCStoryBoardTopComponent win = IPCStoryBoardTopComponent.findInstance();
               win.open();
              // win.addPortletNode(dataObject.getName());
               win.requestActive();
    }
