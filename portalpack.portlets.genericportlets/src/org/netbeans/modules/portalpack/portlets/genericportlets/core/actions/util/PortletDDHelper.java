...
public class PortletDDHelper {
    ...
    private String viewInitParamName;
    private String editInitParamName;
    private String helpInitParamName;
    
    private static Logger logger = Logger.getLogger(CoreUtil.CORE_LOGGER);

    public PortletDDHelper(File inDDFile,String viewParamName,
                                    String editParamName,String helpParamName) {
        ddFile = inDDFile;
        this.viewInitParamName = viewParamName;
        ...
        try {
            portletApp = PortletXMLFactory.createGraph(ddFile);        
     ...
     */
    public void setInitialPage(PortletModeType inMode, String portlet, String inRelativePathPageName) {
        /**
         * First unset the given page so we don't have leftover
         */
        this.unsetInitialPage(portlet, inRelativePathPageName);
        ...
        try {
            ...
            if(null!=portletApp) {
                 ...
                 * we will only deal with the first portlet.
                 */
                PortletType portletType = getPortlet(portlet);
                if(portletType != null) {
                    ...
                    for(int ii=0; null != initParams && ii < initParams.length;ii++) {
                        ...
                        if(inMode.equals(PortletModeType.VIEW)) {
                            if(initParams[ii].getName().equals(viewInitParamName)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        } else if(inMode.equals(PortletModeType.EDIT)) {
                            if(initParams[ii].getName().equals(editInitParamName)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        } else if(inMode.equals(PortletModeType.HELP)) {
                            if(initParams[ii].getName().equals(helpInitParamName)) {
                                foundMode = true;
                                initParams[ii].setValue(inRelativePathPageName);
                            }
                        }
                    ...
                    if(!foundMode) {
                        ...
                        if(inMode.equals(PortletModeType.VIEW)) {
                            initParam.setName(viewInitParamName);
                        } else if(inMode.equals(PortletModeType.EDIT)) {
                            initParam.setName(editInitParamName);
                        } else if(inMode.equals(PortletModeType.HELP)) {
                            initParam.setName(helpInitParamName);
                        }
                     ...
                     */
                   SupportsType [] supportsTypes = portletType.getSupports();
                    ...
                    if(!foundPortletMode) {
                         ...
                         */
                        if(!foundSupports) {
                            supportsType = portletType.newSupportsType();
                        } else {
    ...
    private PortletType getPortlet(String portletName) {
     ...
     * be "/edit/EditPage.jsp".
     */    
    public void unsetInitialPage(String portlet, String inPageName) {
        try {
                ...
                if(portletType != null) {
                    ...
                    for(int ii=0; null != initParams && ii < initParams.length;ii++) {
                        if(initParams[ii].getValue().equals(inPageName)) {
                            portletType.removeInitParam(initParams[ii]);
                    ...
                    for(int ii=0; null != supportsTypes && null != modeToRemove && ii < supportsTypes.length;ii++) {
                        ...
                        if(modeToRemove.equals(viewInitParamName)) {
                            supportsTypes[ii].removePortletMode(PortletModeType.VIEW.toString());
                        } else if(modeToRemove.equals(editInitParamName)) {
                            supportsTypes[ii].removePortletMode(PortletModeType.EDIT.toString());
                        } else if(modeToRemove.equals(helpInitParamName)) {
     ...
     * be "/edit/EditPage.jsp".
     */        
    public boolean isInitialPage(PortletModeType inMode, String portlet, String inPageName) {
     ...
     * null is returned.
     */
    public PortletModeType isInitialPage(String portlet, String inPageName)  {
            ...
            if(portletType != null) {
                ...
                for(int ii=0; null != initParams && ii < initParams.length;ii++) {
                    ...
                    if(null != pageValue && pageValue.equals(inPageName)) {
                        ...
                        } else if (initParams[ii].getName().equals(editInitParamName)) {
                            returnMode = PortletModeType.EDIT;
                        } else if (initParams[ii].getName().equals(helpInitParamName)) {
    ...
    public String getInitialPage(PortletModeType inMode,String portlet) {
    ...
    public void save(PortletApp portletApp,File portletFile)
    {
        try {
            FileObject portletXmlObj = FileUtil.toFileObject(portletFile);
            FileLock lock = portletXmlObj.lock();
            OutputStream out = portletXmlObj.getOutputStream(lock);
