...
public class ThreadsViewNodeModelFilter implements NodeModelFilter {
    ...
    public String getIconBase(NodeModel original, Object node) throws UnknownTypeException {
        if (node instanceof CallStackFrame) {
            CallStackFrame callStackFrame = (CallStackFrame) node;
            JPDADebugger debugger = Utils.getDebugger(callStackFrame);
            if (debugger != null && debugger.getCurrentCallStackFrame() == callStackFrame) {
    ...
    public String getShortDescription(NodeModel original, Object node) throws UnknownTypeException {
        if (node instanceof CallStackFrame) {
            return getDisplayName(original, node);
        }
