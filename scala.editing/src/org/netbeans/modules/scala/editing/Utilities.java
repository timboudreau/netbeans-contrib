...
//    }
    ...
    public static boolean isJavaContext(final JTextComponent component, final int offset) {
        ...
        try {
        TokenSequence<ScalaTokenId> ts = ScalaLexUtilities.getTokenSequence(TokenHierarchy.get(doc), offset);
        if (ts == null)
            return false;        
...
//                    break;
    ...
    private static boolean isClashing(String varName, Iterable<? extends Element> locals) {
        if (LexerScala.SCALA_KEYWORDS.contains(varName))
            return true;
