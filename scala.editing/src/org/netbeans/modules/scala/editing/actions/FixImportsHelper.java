...
//        }
        ...
        ClasspathInfo pathInfo = ScalaUtils.getClasspathInfoForFileObject(fo);
    ...
    int getImportPosition(BaseDocument doc) {
        TokenSequence<?> ts = ScalaLexUtilities.getTokenSequence(doc, 1);
        ...
        while (ts.moveNext()) {
            ...
            if (t.id() == ScalaTokenId.Import) {
                LOG.log(Level.FINEST, "ScalaTokenId.Import found");
                importEnd = offset;
            } else if (t.id() == ScalaTokenId.Package) {
    ...
    public void doImport(FileObject fo, String fqnName) throws MissingResourceException {
        ...
        BaseDocument baseDoc = ScalaLexUtilities.getDocument(fo, true);
