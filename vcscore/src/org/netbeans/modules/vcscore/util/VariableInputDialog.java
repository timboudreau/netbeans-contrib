...
public class VariableInputDialog extends javax.swing.JPanel {
    ...
    public final boolean hasDefaults() {
        ...
        if (inputDescriptor == null || command == null) {
            return false;
        }
    ...
    private int addComponent(final VariableInputComponent component, int gridy,
        ...
        boolean visible = VariableInputComponent.isVarConditionMatch(component.getVarConditions(), vars);
        if (visible) {
            visible = (component.isTrivial() && !expert);
    ...
    private int addSelectRadio(final VariableInputComponent component, int gridy,
        ...
        for (Enumeration en = group.getElements(); en.hasMoreElements(); ) {
            java.awt.Component c = (java.awt.Component) en.nextElement();
            if (setMainComponent) {
                ...
                if (!button.isEnabled()) { // The button was just disabled
                    if (button.isSelected()) {
                        ...
                        for (int i = 0; en.hasMoreElements(); i++) {
                            javax.swing.JRadioButton radio = (javax.swing.JRadioButton) en.nextElement();
                            if (radio.isEnabled()) {
        ...
        addActionToProcess(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                ...
                if (selected2 < 0) {
                    ...
                    selectedValue = subComponents[selected2].getValue();
                }
	...
	this.userPromptLabelTexts = new String[varLabels.size()];
        ...
        for(Enumeration en = varLabels.keys(); en.hasMoreElements(); i++) {
            String labelStr = (String) en.nextElement();
    ...
    private void doPreprocessComponents() {
        if (componentsToPreprocess.size() > 0) {
            ...
            Hashtable preCommandVars = new Hashtable(vars);
            PreCommandPerformer cmdPerf = new PreCommandPerformer(executionContext, preCommandVars);
            try {
