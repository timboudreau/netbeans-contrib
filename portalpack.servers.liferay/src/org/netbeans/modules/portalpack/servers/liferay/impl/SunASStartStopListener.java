...
public class SunASStartStopListener implements ServerStartStopListener{
    ...
    private boolean installRequiredForPC() {
        ...
        try {
            psconfig.setAndSaveProperty(LiferayConstants.SETUP_DONE,"true");
            if(psconfig.getProperty(LiferayConstants.SETUP_DONE) != null &&
                    psconfig.getProperty(LiferayConstants.SETUP_DONE).trim().equals("true")) {
                url = new URL("http://" + psconfig.getHost() + ":" + psconfig.getPort() + "/"
                        + psconfig.getProperty(LiferayConstants.ADMIN_CONSOLE_URI));
