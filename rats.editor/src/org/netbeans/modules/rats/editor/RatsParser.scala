...
import xtc.tree.{Node,Location}

import org.netbeans.modules.rats.editor.lexer.{RatsTokenId,LexUtil}
import org.netbeans.modules.rats.editor.lexer.RatsTokenId._
import org.netbeans.modules.rats.editor.rats.ParserRats

/**
 *
 * @author Caoyuan Deng
 */
class RatsParser extends Parser {

  private var lastResult :RatsParserResult = _
  ...
  private def sanitize(context:Context, sanitizing:Sanitize) :RatsParserResult = {
    sanitizing match {
      case NEVER =>
  ...
  protected def parseBuffer(context:Context, sanitizing:Sanitize) :RatsParserResult = {
    ...
    try {
      ...
      val r = parser.pModule(0)
      if (r.hasValue) {
        val v = r.asInstanceOf[SemanticValue]
        root = Some(v.value.asInstanceOf[Node])
      } else {
  ...
  protected def createParser(context:Context) :ParserRats = {
    val in = new StringReader(context.source)
    new ParserRats(in, fileName(context))
  }

  private def createParseResult(context:Context) :RatsParserResult = {
    new RatsParserResult(context.snapshot, context.root, None/*context.rootScope*/)
  }
  ...
  class Context(val snapshot:Snapshot, event:SourceModificationEvent) {
    ...
    var source :String = RatsParser.asString(snapshot.getText)
