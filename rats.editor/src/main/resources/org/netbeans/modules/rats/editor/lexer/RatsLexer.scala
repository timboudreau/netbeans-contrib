...
import org.netbeans.modules.rats.editor.rats.LexerRats
...
object RatsLexer {
   ...
   */
  def create(info:LexerRestartInfo[TokenId]) = new RatsLexer(info)
}

class RatsLexer(info:LexerRestartInfo[TokenId]) extends Lexer[TokenId] {
  /** @Note:
  ...
  def createToken(id:TokenId, length:Int) :Token[TokenId] = id.asInstanceOf[RatsTokenId].fixedText match {
  ...
  def scanTokens :Result = {
     ...
     * difficult to handle.
     */
    val scanner = new LexerRats(lexerInputReader, "<current>")
    try {
    ...
    while (i < l) {
      node.get(i) match {
        ...
        case child:String =>
          ...
          val id = RatsTokenId.valueOf(node.getName) match {
            case None => RatsTokenId.IGNORED
