    ...
    public void reindent(final Context context) {
        if (context.startOffset() == context.endOffset()) {
            ...
            document.runAtomic(new Runnable() {
                ...
                public void run() {
                    try {
                        int rowIndent = Utilities.getRowIndent(document, context.startOffset() - 1);
                        if(rowIndent != -1) {
                            String indentString = IndentUtils.createIndentString(document, rowIndent);
                            document.insertString(context.caretOffset(), indentString, null);
                        }
