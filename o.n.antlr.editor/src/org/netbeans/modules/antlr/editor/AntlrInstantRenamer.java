 ...
 * @author marekfukala
 */
public class AntlrInstantRenamer implements InstantRenamer {
    ...
    public boolean isRenameAllowed(ParserResult info, int caretOffset, String[] explanationRetValue) {
        ...
        return curr != null && (curr.id() == AntlrTokenId.TOKEN_REF || curr.id() == AntlrTokenId.RULE_REF);
    }
                ...
                case RULE_REF:
                    ...
                    while (ts.moveNext()) {
                        ...
                        if (token.id() == curr.id() && Utils.equals(token.text(), curr.text(), false, false)) {
                            regions.add(new OffsetRange(ts.offset(), ts.offset() + token.length()));
                        }
                    }
                    break;                    
            }
