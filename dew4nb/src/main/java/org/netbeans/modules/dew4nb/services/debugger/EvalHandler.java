...
public class EvalHandler extends BasicRequestHandler<DebugAction, DebugMessageType, EvalResult> {

    public EvalHandler() {
        super(DebugerModels.END_POINT, DebugMessageType.eval, DebugAction.class, EvalResult.class);
    }

    @Override
    protected Status handle(DebugAction request, EvalResult response) {
        Parameters.notNull("request", request); //NOI18N
        Parameters.notNull("response", response);   //NOI18N
        if (request.getType() != DebugMessageType.eval) {
