 ...
 * @author Tomas Zezula
 */
final class RemoteSourceFileObject extends AbstractSourceFileObject {

    private final boolean hasFilter;

    RemoteSourceFileObject(
    ...
    @Override
    @CheckForNull
    protected Long isDirty() {
        return getRemoteContent() != null ?
    ...
    @Override
    @NonNull
    protected CharSequence createContent() throws IOException {
        CharSequence content = getRemoteContent();
        if (content == null) {
            ...
            content = source.createSnapshot().getText();
    ...
    @CheckForNull
    private CharSequence getRemoteContent() {
        final FileObject file = getHandle().resolveFileObject(false);
        return file == null ?
            null :
            SourceProvider.getInstance().getContentFor(file);
    }
