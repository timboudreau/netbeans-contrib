...
final class JavacServer {
    ...
    public synchronized static JavacServer getDefault() {
        if (INSTANCE == null) {
            final String web4dew = System.getProperty("web4dew"); //NOI18N
            final File root;
            final boolean cache;
            if (web4dew == null) {
                final File index = InstalledFileLocator.getDefault().locate("web4dew/index.html", "org.netbeans.modules.dew4nb", true);
                root = index.getParentFile();
                cache = true;
            } else {
                root = new File(web4dew);
                cache = false;
            }
            INSTANCE = new JavacServer(
                Server.createBuilder().
                setPortRange(9000, 65535).
                addStaticHttpRoot(root, cache).
                build());
        }
        return INSTANCE;
    }
