...
import org.netbeans.modules.debugger.javafx.EditorContextBridge;
import org.netbeans.modules.debugger.javafx.ExpressionPool.Expression;
...
public class RunIntoMethodActionProvider extends ActionsProviderSupport 
    ...
    public RunIntoMethodActionProvider(ContextProvider lookupProvider) {
        ...
        debugger.addPropertyChangeListener (JavaFXDebuggerImpl.PROP_STATE, this);
        EditorContextBridge.getContext().addPropertyChangeListener (this);
    }
    
    private void destroy () {
        debugger.removePropertyChangeListener (JavaFXDebuggerImpl.PROP_STATE, this);
    ...
    public void doAction (Object action) {
        ...
        if (!classes.isEmpty()) {
            ...
            cbrkp.addJavaFXBreakpointListener(new JavaFXBreakpointListener() {
    ...
    private void resume() {
        if (debugger.getSuspend() == JavaFXDebugger.SUSPEND_EVENT_THREAD) {
    ...
    private void traceLineForMethod(final String method, final int methodLine) {
        ...
        final JavaFXStep step = debugger.createJavaFXStep(JavaFXStep.STEP_LINE, JavaFXStep.STEP_INTO);
        step.setHidden(true);
        step.addPropertyChangeListener(JavaFXStep.PROP_STATE_EXEC, new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                if (currentDepth == depth) { // We're in the outer expression
                    try {
                        if (t.getCallStack()[0].getLineNumber("JavaFX") != methodLine) {
                            ...
                            step.setDepth(JavaFXStep.STEP_INTO);
                    ...
                    if (t.getMethodName().equals(method)) {
                        ...
                        step.setDepth(JavaFXStep.STEP_OUT);
                        step.addStep(debugger.getCurrentThread());
                    }
                }
            }
