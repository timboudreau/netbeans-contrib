...
implements Executor, PropertyChangeListener {
    ...
    public StepIntoActionProvider (ContextProvider contextProvider) {
        super (
            (JavaFXDebuggerImpl) contextProvider.lookupFirst 
    ...
    public void runAction() {
        synchronized (getDebuggerImpl ().LOCK) {
            ...
            JavaFXThread resumeThread = setStepRequest (StepRequest.STEP_INTO);
    ...
    protected void checkEnabled (int debuggerState) {
        ...
        while (i.hasNext ())
            setEnabled (
                i.next (),
                (debuggerState == JavaFXDebugger.STATE_STOPPED) &&
    ...
    public void propertyChange (PropertyChangeEvent ev) {
        if (ev.getPropertyName () == SmartSteppingFilter.PROP_EXCLUSION_PATTERNS) {
            if (ev.getOldValue () != null) {
                ...
                JavaFXThreadImpl currentThread = (JavaFXThreadImpl) getDebuggerImpl().getCurrentThread();
        ...
        if (ev.getPropertyName () == SourcePathProvider.PROP_SOURCE_ROOTS) {
            ...
            JavaFXThreadImpl jtr = (JavaFXThreadImpl) getDebuggerImpl ().
                getCurrentThread ();
            if (jtr != null) {
    ...
    public boolean exec (Event event) {
        ...
        JavaFXThread resumeThread = null;
        synchronized (getDebuggerImpl ().LOCK) {
            if (stepRequest != null) {
                stepRequest.disable ();
            }
    ...
    private JavaFXThreadImpl setStepRequest (int step) {
        JavaFXThreadImpl thread = (JavaFXThreadImpl) getDebuggerImpl().getCurrentThread();
