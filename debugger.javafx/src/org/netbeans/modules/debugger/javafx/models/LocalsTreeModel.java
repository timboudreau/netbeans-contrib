...
public class LocalsTreeModel implements TreeModel, PropertyChangeListener {
    ...
    throws UnknownTypeException {
        try {
            ...
            if (o instanceof JavaFXClassType) {
                JavaFXClassType clazz = (JavaFXClassType) o;
            ...
            if (o instanceof String && ((String) o).startsWith("operationArguments")) { // NOI18N
                ...
                List<org.netbeans.api.debugger.javafx.LocalVariable> arguments;
                try {
    ...
    public int getChildrenCount (Object node) throws UnknownTypeException {
        try {
            if (node.equals (ROOT)) {
                ...
                try {
                    ...
                    if (operations != null && operations.size() > 0 && operations.get(0).getReturnValue() != null) {
                        ...
                        returnVariable = ((JavaFXThreadImpl) frame.getThread()).getReturnVariable();
            ...
            if (node instanceof JavaFXClassType) {
                // Performance, see issue #59058.
                return Integer.MAX_VALUE;
                //JavaFXClassType clazz = (JavaFXClassType) node;
        ...
        synchronized (debugger.LOCK) {
            ...
            try {
                ...
                if (operations != null && operations.size() > 0 && operations.get(0).getReturnValue() != null) {
                    ...
                    returnVariable = ((JavaFXThreadImpl) callStackFrame.getThread()).getReturnVariable();
                    haveLastOperations = false;
                }
    ...
    private org.netbeans.api.debugger.javafx.LocalVariable[] getLocalVariables (
        final CallStackFrameImpl    callStackFrame, 
        final StackFrame            stackFrame,
        int                         from,
        int                         to
    ) {
        org.netbeans.api.debugger.javafx.LocalVariable[] locals;
        try {
        ...
        if (locals == null) {
            locals = new org.netbeans.api.debugger.javafx.LocalVariable[] {};
        }
        int n = locals.length;
        to = Math.min(n, to);
        from = Math.min(n, from);
        if (from != 0 || to != n) {
            org.netbeans.api.debugger.javafx.LocalVariable[] subLocals = new org.netbeans.api.debugger.javafx.LocalVariable[to - from];
        ...
        if (local != null) {
            ...
            if (v instanceof ObjectReference) {
                local = new ObjectLocalVariable (
                    ...
                    className, 
                    lv, 
                    JavaFXDebuggerImpl.getGenericSignature (lv), 
                    frame
                );
            } else {
    ...
    private void updateVarListeners(org.netbeans.api.debugger.javafx.LocalVariable[] vars) {
        varListeners = new PropertyChangeListener[vars.length];
        for (int i = 0; i < vars.length; i++) {
            final org.netbeans.api.debugger.javafx.LocalVariable var = vars[i];
    ...
    private static class Listener implements PropertyChangeListener {
        ...
        public void propertyChange (PropertyChangeEvent e) {
            if ( ( (e.getPropertyName () == 
                     JavaFXDebugger.PROP_CURRENT_CALL_STACK_FRAME) ||
                   //(e.getPropertyName () == debugger.PROP_CURRENT_THREAD) ||
                   (e.getPropertyName () == JavaFXDebugger.PROP_STATE)
