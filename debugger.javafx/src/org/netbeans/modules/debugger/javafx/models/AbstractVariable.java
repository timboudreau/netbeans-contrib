...
import org.netbeans.modules.debugger.javafx.expr.EvaluatorVisitor;
    ...
    */
    ...
    private Value convertValue(Value value, Type type) {
        if (type instanceof PrimitiveType) {
            if (value instanceof ObjectReference) {
                JavaFXThread ct = getDebugger().getCurrentThread();
                if (ct != null) {
                    try {
                        value = EvaluatorVisitor.unbox((ObjectReference) value,
                                                       (PrimitiveType) type,
                                                       ((JavaFXThreadImpl) ct).getThreadReference());
    ...
    public JavaFXClassType getClassType() {
        ...
        if (type instanceof ReferenceType) {
            return new JavaFXClassTypeImpl(debugger, (ReferenceType) type);
        } else {
            return null;
        }
    }
    ...
    protected final JavaFXDebuggerImpl getDebugger() {
        return debugger;
    }
