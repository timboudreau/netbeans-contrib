...
public class FieldBreakpointImpl extends ClassBasedBreakpoint {
    ...
    public FieldBreakpointImpl (FieldBreakpoint breakpoint, JavaFXDebuggerImpl debugger, Session session) {
    ...
    private void setFilters(WatchpointRequest wr) {
        ...
        if (threadFilters != null && threadFilters.length > 0) {
            for (JavaFXThread t : threadFilters) {
                wr.addThreadFilter(((JavaFXThreadImpl) t).getThreadReference());
            }
        }
