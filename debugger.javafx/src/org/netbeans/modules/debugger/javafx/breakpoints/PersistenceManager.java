...
public class PersistenceManager implements LazyDebuggerManagerListener {
    ...
    public synchronized Breakpoint[] unloadBreakpoints() {
        ...
        for (Breakpoint b : bpts) {
            if (b instanceof JavaFXBreakpoint) {
                unloaded.add(b);
    ...
    public void breakpointAdded (Breakpoint breakpoint) {
        if (breakpoint instanceof JavaFXBreakpoint &&
                !((JavaFXBreakpoint) breakpoint).isHidden ()) {
    ...
    public void propertyChange (PropertyChangeEvent evt) {
        if (evt.getSource() instanceof JavaFXBreakpoint) {
            if (LineBreakpoint.PROP_LINE_NUMBER.equals(evt.getPropertyName())) {
                ...
                if (r != null) {
                    ...
                    r.storeCachedClassName((JavaFXBreakpoint) evt.getSource(), null);
                }
            }
    ...
    static BreakpointsReader findBreakpointsReader() {
        ...
        while (i.hasNext ()) {
            ...
            if (ns.length == 1 && JavaFXBreakpoint.class.getName().equals(ns[0])) {
    ...
    private static Breakpoint[] getBreakpoints () {
        ...
        for (i = 0; i < k; i++)
            ...
            if ( bs[i] instanceof JavaFXBreakpoint &&
                 !((JavaFXBreakpoint) bs [i]).isHidden ()
            )
                bb.add (bs [i]);
