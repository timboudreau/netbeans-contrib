...
import org.netbeans.modules.debugger.javafx.expr.JDIVariable;
...
public class MethodBreakpointImpl extends ClassBasedBreakpoint {
    ...
    public MethodBreakpointImpl (MethodBreakpoint breakpoint, JavaFXDebuggerImpl debugger, Session session) {
    ...
    protected EventRequest createEventRequest(EventRequest oldRequest) {
        ...
        if (oldRequest instanceof MethodEntryRequest) {
            ...
            JavaFXThread[] threadFilters = breakpoint.getThreadFilters(getDebugger());
            if (threadFilters != null && threadFilters.length > 0) {
                for (JavaFXThread t : threadFilters) {
                    entryReq.addThreadFilter(((JavaFXThreadImpl) t).getThreadReference());
                }
            }
        ...
        if (oldRequest instanceof MethodExitRequest) {
            ...
            if (threadFilters != null && threadFilters.length > 0) {
                for (JavaFXThread t : threadFilters) {
                    exitReq.addThreadFilter(((JavaFXThreadImpl) t).getThreadReference());
                }
            }
            ObjectVariable[] varFilters = breakpoint.getInstanceFilters(getDebugger());
            if (varFilters != null && varFilters.length > 0) {
