...
public final class BreakpointsReader implements Properties.Reader, PropertyChangeListener {
    
private Map<JavaFXBreakpoint, String> cachedClassNames = new WeakHashMap<JavaFXBreakpoint, String>();
    private Map<JavaFXBreakpoint, String> cachedSourceRoots = new WeakHashMap<JavaFXBreakpoint, String>();
    
    
    public String [] getSupportedClassNames () {
        return new String[] {
            JavaFXBreakpoint.class.getName (), 
        };
    }
    
    synchronized String findCachedClassName(JavaFXBreakpoint b) {
        return cachedClassNames.get(b);
    }
    
    synchronized String findCachedSourceRoot(JavaFXBreakpoint b) {
        return cachedSourceRoots.get(b);
    }
    
    void storeCachedClassName(JavaFXBreakpoint b, String className) {
        synchronized (this) {
    ...
    void storeCachedSourceRoot(JavaFXBreakpoint b, String sourceRoot) {
        synchronized (this) {
    ...
    public Object read (String typeID, Properties properties) {
        ...
        b.setPrintText (
            properties.getString (JavaFXBreakpoint.PROP_PRINT_TEXT, "")
        );
        b.setGroupName(
            properties.getString (JavaFXBreakpoint.PROP_GROUP_NAME, "")
        );
        b.setSuspend (
            properties.getInt (
                JavaFXBreakpoint.PROP_SUSPEND, 
                JavaFXBreakpoint.SUSPEND_ALL
            )
        );
        int hitCountFilter = properties.getInt(JavaFXBreakpoint.PROP_HIT_COUNT_FILTER, 0);
        ...
        if (hitCountFilter > 0) {
            hitCountFilteringStyle = Breakpoint.HIT_COUNT_FILTERING_STYLE.values()
                    [properties.getInt(JavaFXBreakpoint.PROP_HIT_COUNT_FILTER+"_style", 0)]; // NOI18N
        } else {
            hitCountFilteringStyle = null;
        }
        b.setHitCountFilter(hitCountFilter, hitCountFilteringStyle);
        if (properties.getBoolean (JavaFXBreakpoint.PROP_ENABLED, true))
            b.enable ();
        else
            b.disable ();
        return b;
    ...
    public void write (Object object, Properties properties) {
        JavaFXBreakpoint b = (JavaFXBreakpoint) object;
        properties.setString (
            JavaFXBreakpoint.PROP_PRINT_TEXT, 
            b.getPrintText ()
        );
        properties.setString (
            JavaFXBreakpoint.PROP_GROUP_NAME, 
            b.getGroupName ()
        );
        properties.setInt (JavaFXBreakpoint.PROP_SUSPEND, b.getSuspend ());
        properties.setBoolean (JavaFXBreakpoint.PROP_ENABLED, b.isEnabled ());
        properties.setInt(JavaFXBreakpoint.PROP_HIT_COUNT_FILTER, b.getHitCountFilter());
        Breakpoint.HIT_COUNT_FILTERING_STYLE style = b.getHitCountFilteringStyle();
        properties.setInt(JavaFXBreakpoint.PROP_HIT_COUNT_FILTER+"_style", style != null ? style.ordinal() : 0); // NOI18N
