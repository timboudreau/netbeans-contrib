...
import org.netbeans.modules.debugger.javafx.models.ReturnVariableImpl;
import org.netbeans.modules.debugger.javafx.util.Executor;
import org.netbeans.modules.debugger.javafx.util.ThreadInfoPanel;
...
public abstract class BreakpointImpl implements Executor, PropertyChangeListener {
    ...
    protected BreakpointImpl (JavaFXBreakpoint p, BreakpointsReader reader, JavaFXDebuggerImpl debugger, Session session) {
    ...
    final void update () {
        if ( (getVirtualMachine () == null) ||
             (getDebugger ().getState () == JavaFXDebugger.STATE_DISCONNECTED)
        ) return;
    ...
    protected JavaFXBreakpoint getBreakpoint () {
        return breakpoint;
    }

    protected JavaFXDebuggerImpl getDebugger () {
        return debugger;
    }
    ...
    synchronized protected void addEventRequest (EventRequest r, boolean ignoreHitCount) {
        ...
        if (getBreakpoint().getSuspend() == JavaFXBreakpoint.SUSPEND_ALL)
            r.setSuspendPolicy (JavaFXBreakpoint.SUSPEND_ALL);
        else
            r.setSuspendPolicy (JavaFXBreakpoint.SUSPEND_EVENT_THREAD);
    ...
    protected boolean perform (
        ...
        if (getBreakpoint() instanceof MethodBreakpoint &&
            ...
            JavaFXThreadImpl jt = (JavaFXThreadImpl) getDebugger().getThread(thread);
            if (value != null) {
        ...
        if ((condition == null) || condition.equals ("")) {
            JavaFXBreakpointEvent e = new JavaFXBreakpointEvent (
                getBreakpoint (),
                debugger,
                JavaFXBreakpointEvent.CONDITION_NONE,
            ...
            resume = getBreakpoint().getSuspend() == JavaFXBreakpoint.SUSPEND_NONE || e.getResume ();
            ...
            //PATCH 48174
            resume = getBreakpoint().getSuspend() == JavaFXBreakpoint.SUSPEND_NONE || resume;
        }
    ...
    private boolean checkWhetherResumeToFinishStep(ThreadReference thread) {
        ...
        if (stepRequests.size() > 0) {
            ...
            if (suspendState == JavaFXBreakpoint.SUSPEND_ALL ||
                suspendState == JavaFXBreakpoint.SUSPEND_EVENT_THREAD) {
                ...
                if (activeStepRequests.size() > 0 && (thisThreadHasStep || suspendState == JavaFXBreakpoint.SUSPEND_ALL)) {
    ...
    private boolean evaluateCondition (
        ...
        try {
            try {
                boolean result;
                JavaFXBreakpointEvent ev;
                synchronized (debugger.LOCK) {
                    StackFrame sf = thread.frame (0);
                    result = evaluateConditionIn (condition, sf, 0);
                    ev = new JavaFXBreakpointEvent (
                        getBreakpoint (),
                        debugger,
                        result ? 
                            JavaFXBreakpointEvent.CONDITION_TRUE : 
                            JavaFXBreakpointEvent.CONDITION_FALSE,
                        debugger.getThread (thread), 
                        referenceType, 
                        variable
                    );
                ...
                JavaFXBreakpointEvent ev = new JavaFXBreakpointEvent (
                    getBreakpoint (),
                    debugger,
                    ex,
                    debugger.getThread (thread), 
