...
class BracketCompletion {
  ...
  static void charInserted(BaseDocument doc,  
      ...
      if (ch == ')'|| ch == ']'|| ch =='('|| ch =='[') {
          ...
          if (tokenAtDot == JavaFXTokenContext.RBRACKET || tokenAtDot == JavaFXTokenContext.RPAREN) {
              skipClosingBracket(doc, caret, ch);
          } else if (tokenAtDot == JavaFXTokenContext.LBRACKET || tokenAtDot == JavaFXTokenContext.LPAREN) {
    ...
    private static void moveSemicolon(BaseDocument doc, int dotPos, Caret caret) throws BadLocationException {
        ...
        for (TokenItem item = token.getNext(); item != null && item.getOffset() <= eolPos; item = item.getNext()) {
            ...
            if (tokenID == JavaFXTokenContext.RPAREN) {
                lastParenPos = item.getOffset();
            } else if (tokenID != JavaFXTokenContext.WHITESPACE) {
                return;
            }
        }
    ...
    private static boolean isForLoopSemicolon(TokenItem token) {
        if (token == null || token.getTokenID() != JavaFXTokenContext.SEMICOLON) {
            return false;
        }
        ...
        while (token != null) {
            if (token.getTokenID() == JavaFXTokenContext.LPAREN) {
                if (parDepth == 0) { // could be a 'for ('
                    token = token.getPrevious();
                    while(token !=null && (token.getTokenID() == JavaFXTokenContext.WHITESPACE || token.getTokenID() == JavaFXTokenContext.BLOCK_COMMENT || token.getTokenID() == JavaFXTokenContext.LINE_COMMENT)) {
                        token = token.getPrevious();
                    }
                    if (token.getTokenID() == JavaFXTokenContext.FOR) {
                        return true;
                    }
                    return false;
                } else { // non-zero depth
            ...
            } else if (token.getTokenID() == JavaFXTokenContext.RPAREN) {
                parDepth++;
            } else if (token.getTokenID() == JavaFXTokenContext.LBRACE) {
                if (braceDepth == 0) { // unclosed left brace
                    return false;
                }
                braceDepth--;
            } else if (token.getTokenID() == JavaFXTokenContext.RBRACE) {
                braceDepth++;

            } else if (token.getTokenID() == JavaFXTokenContext.SEMICOLON) {
			     ...
			     char ch) throws BadLocationException 
  {
	...
	if ((tokenAtDot == JavaFXTokenContext.RBRACKET && tokenBalance(doc, JavaFXTokenContext.LBRACKET, JavaFXTokenContext.RBRACKET) != 0) ||
	    (tokenAtDot == JavaFXTokenContext.RPAREN && tokenBalance(doc, JavaFXTokenContext.LPAREN, JavaFXTokenContext.RPAREN) != 0) ) {
	  doc.remove(dotPos, 1);
	}
      } 
      else if (ch == '\"') {
        ...
        if (completionSettingEnabled()) {
            if (caretOffset > 0) {
                ...
                if (addRightBrace) { // still candidate for adding
                    ...
                    if ((token == null)
                    || (!token.getTokenID().equals(JavaFXTokenContext.LBRACE))
                    || (token.getOffset() < caretRowStartOffset)
                    ) {
                        addRightBrace = false;
                    }
  ...
  private static int braceBalance(BaseDocument doc) 
    throws BadLocationException
  {
    return tokenBalance(doc, JavaFXTokenContext.LBRACE, JavaFXTokenContext.RBRACE);
  }
  ...
  throws BadLocationException {
      ...
      if (token != null && token.getTokenID() == bracketId) {
          ...
          int leftBracketIntId = (bracketIntId == JavaFXTokenContext.RPAREN_ID)
          ? JavaFXTokenContext.LPAREN_ID
          : JavaFXTokenContext.LBRACKET_ID;
  ...
  static boolean completeQuote(BaseDocument doc, int dotPos, Caret caret,
    ...
    if (token == JavaFXTokenContext.BLOCK_COMMENT || token == JavaFXTokenContext.LINE_COMMENT){
        return false;
    } else if (token == JavaFXTokenContext.WHITESPACE && eol && dotPos-1 > 0){
        // check if the caret is at the very end of the line comment
        token = syntax.getTokenID(dotPos-1);
        if (token == JavaFXTokenContext.LINE_COMMENT){
            return false;
        }
    }
    ...
    boolean insideString = 
            token == JavaFXTokenContext.STRING_LITERAL ||
            token == JavaFXTokenContext.CHAR_LITERAL;
    
    if (!insideString){
        // check if the caret is at the very end of the line and there
        // is an unterminated string literal
        if (token == JavaFXTokenContext.WHITESPACE && eol){
            if (dotPos-1 > 0){
                token = syntax.getTokenID(dotPos-1);
                insideString = 
                    token == JavaFXTokenContext.STRING_LITERAL ||
