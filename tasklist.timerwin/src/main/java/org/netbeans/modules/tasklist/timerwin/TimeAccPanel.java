...
public class TimeAccPanel extends javax.swing.JPanel {
    ...
    // Mavenization - attempting to blindly implement both interfaces this might use - this module
    // drifted away from compatibility with the rest of tasklist and probably was abandoned earlier
    class OLL implements ObjectListListener, org.netbeans.modules.tasklist.usertasks.util.ObjectListListener {

        @Override
        public void listChanged(ObjectListEvent e) {
            if (e.getType() == ObjectListEvent.EVENT_REMOVED) {
        ...
        @Override
        public void listChanged(org.netbeans.modules.tasklist.usertasks.util.ObjectListEvent e) {
            if (e.getType() == org.netbeans.modules.tasklist.usertasks.util.ObjectListEvent.EVENT_REMOVED) {
                Object[] list = e.getObjects();
                for (int i = 0; i < list.length; i++) {
                    int index = findUserTask((UserTask) list[i]);
                    if (index >= 0)
