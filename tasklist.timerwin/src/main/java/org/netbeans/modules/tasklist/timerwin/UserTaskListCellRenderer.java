...
public class UserTaskListCellRenderer extends DefaultListCellRenderer {
        ...
        boolean cellHasFocus)
    {
        ...
        if (value == null) {
            ...
            Duration d = new Duration(ut.getSpentTime(), set.getMinutesPerDay() / 60,
                    set.getDaysPerWeek());
            setText("[" + d.days + "D " + d.hours + ":" + d.minutes + "] " +  ut.getSummary());
            // Pre-mavenization code:
//            icon.setImage(UserTaskIconProvider.getUserTaskImage(
//                    (UserTask) value, false));
//            setIcon(icon);
            setIcon(UserTaskIconProvider.getUserTaskImage(
                    (UserTask) value, false));
        }
        return this;
    }
