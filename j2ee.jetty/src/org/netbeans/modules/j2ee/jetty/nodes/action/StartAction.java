...
/**
 * Class representing action that starts a web module
 * @author novakm
 */
public class StartAction extends NodeAction {
		...
		for (int i = 0; i < activatedNodes.length; i++) {
			...
			if (cookie != null) {
				cookie.start();
                        ...
                        RefreshWebModulesCookie refCookie = (RefreshWebModulesCookie) activatedNodes[i].getParentNode().getCookie(RefreshWebModulesCookie.class);
                        if (refCookie != null)
                            refCookie.refresh();
                }
	}
	...
	protected boolean enable(Node[] activatedNodes) {
		JetWebModuleCookie cookie;
		for (int i = 0; i < activatedNodes.length; i++) {
			cookie = (JetWebModuleCookie) activatedNodes[i].getCookie(JetWebModuleCookie.class);
