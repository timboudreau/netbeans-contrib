...
class ClassPathProviderImpl implements ClassPathProvider {
    ...
    public ClassPath findClassPath(FileObject file, String type) {
        ...
        File root = f;
        while (root != null) {
            if (Cache.get(root + WebCacheConstants.DOCROOT) != null) {
                break;
            }
        ...
        public List<? extends PathResourceImplementation> getResources() {
            ...
            synchronized (this) {
                ...
                if (type.equals(ClassPath.SOURCE)) {
                    path = Cache.get(root + WebCacheConstants.DOCROOT);
                } else if (type.equals(ClassPath.COMPILE)) {
                    path = Cache.get(root + WebCacheConstants.CLASSPATH);
// TODO: does web need to provide these? probably yes
//                } else if (type.equals(ClassPath.EXECUTE)) {
//                    String classpath = Cache.get(root + WebCacheConstants.CLASSPATH);
//                    String binary = Cache.get(root + WebCacheConstants.BINARY);
//                    path = classpath != null ?
//                        (binary != null ? classpath + File.pathSeparator + binary : classpath) :
//                            (binary != null ? binary : null);
                } else if (type.equals(ClassPath.BOOT)) {
                    path = Cache.get(root + WebCacheConstants.BOOTCLASSPATH);
                }
                if (path != null) {
    ...
    void open() {
        ...
        synchronized (registeredPaths) {
            ...
            for (SourceGroup g : ProjectUtils.getSources(prj).getSourceGroups(SourcesImpl.SOURCES_TYPE_DOCROOT)) {
