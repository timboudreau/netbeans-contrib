...
public class InstallAction extends WizardAction {
    ...
    public void execute() {
        ...
        final Map<Product, Integer> weights = new HashMap<Product, Integer>();

        long weight = 0;
        for (Product product : products) {
            weight += product.getDataUris().size();
        }
        
        for (Product product : products) {
            weights.put(product, (int)(100 * product.getDataUris().size() / weight));
        }
       ...
       // overallProgress.setPercentage(percentageLeak);
        overallProgress.synchronizeDetails(false);
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            LogManager.log("Setting " +product.getDisplayName() + " with weight = " + weights.get(product));
            overallProgress.addChild(currentProgress, weights.get(product));
