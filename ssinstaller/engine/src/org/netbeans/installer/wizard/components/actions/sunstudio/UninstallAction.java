...
public class UninstallAction extends WizardAction {
    ...
    public void execute() {
        ...
        /*
         * Setting percents for all components 
         */
        final Map<Product, Integer> percents = new HashMap<Product, Integer>();
        int allProductSize = 0;
        for (Product product : products) {
            allProductSize += product.getDataUris().size();
        }        
        for (Product product : products) {
            percents.put(product, (Progress.COMPLETE - Progress.START) 
                    * product.getDataUris().size() / allProductSize);
        }
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            overallProgress.addChild(currentProgress, percents.get(product));                     
