...
public class Solaris9CPUInfoParser extends SolarisCPUInfoParser {
    ...
    protected String extractCPUInformation() {
        if (info == null) {
            Vector<String> lines = parseInput("-vp");
            if (lines != null && !lines.isEmpty()) {
                String[] fields = lines.get(0).split(" ");
                if (fields.length > 2) info = fields[1].trim();
            }
        }
        return info;
    }
    ...
    protected float extractSpeedInformation() {
        if (speed == 0) {
            Vector<String> lines = parseInput("-v");
            if (lines != null && lines.size() >= 2) {
                String[] fields = lines.get(2).split(" ");
                if (fields.length >= 2) speed = Float.parseFloat(fields[fields.length - 2].trim());
            }            
        }
        return speed;
    }
