 ...
 */
public class AdaSharabilityQuery implements SharabilityQueryImplementation, PropertyChangeListener  {
    ...
    AdaSharabilityQuery (final AntProjectHelper helper, final PropertyEvaluator evaluator, final SourceRoots sources, final SourceRoots tests) {
    ...
    public int getSharability(final File file) {
        ...
        return ProjectManager.mutex().readAccess(new Mutex.Action<Integer>() {
            public Integer run() {
                synchronized (AdaSharabilityQuery.this) {
                    if (delegate == null) {
