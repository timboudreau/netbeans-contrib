...
public final class SourceRoots {
            ...
            ProjectManager.mutex().writeAccess(
                    new Mutex.ExceptionAction<Void> () { 
                        public Void run() throws IOException {
                            ...
                            NodeList nl = cfgEl.getElementsByTagNameNS(AdaProjectType.PROJECT_CONFIGURATION_NAMESPACE, elementName);
                            assert nl.getLength() == 1 : "Illegal project.xml"; //NOI18N
                            Element ownerElement = (Element) nl.item(0);
                            // remove all old roots
                            NodeList rootsNodes =
                                    ownerElement.getElementsByTagNameNS(AdaProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root");    //NOI18N
                            ...
                            for (URL newRoot : newRoots) {
                                ...
                                Element newRootNode = doc.createElementNS(AdaProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root"); //NOI18N
    ...
    private void readProjectMetadata() {
        ...
        if (nl.getLength() == 1) {
            NodeList roots =
                    ((Element) nl.item(0)).getElementsByTagNameNS(AdaProjectType.PROJECT_CONFIGURATION_NAMESPACE, "root"); //NOI18N
