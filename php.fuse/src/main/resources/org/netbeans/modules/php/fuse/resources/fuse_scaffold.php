...
$parametersRun = false;
// if number of parameters is enough for command line input
if (sizeof($argv) > 3) $parametersRun = true;
...
if (!$parametersRun) {
    ...
    if ( !is_dir($structure->base_path && !$parametersRun) ) {
...
if (!$parametersRun) {
    ...
    if ($argv[3] == 'y') {
        $structure->db_config['default']['hostname'] = $argv[4];
        $structure->db_config['default']['username'] = $argv[5];
        $structure->db_config['default']['password'] = $argv[6];
        $structure->db_config['default']['db_name'] = $argv[7];
    }
}
...
if (!$parametersRun) {
    ...
    isset($argv[8])? $structure->fuse_base_path = $argv[8]: $structure->fuse_base_path = $argv[4];
}
$answeredYes = false;
if (!$parametersRun) {
    echo "\n" . 'FUSE project will be created in ' . $structure->base_path . '. Continue [Y/N]?';
    $answeredYes = user_answered_yes();
}
if ( $answeredYes || $parametersRun) {
