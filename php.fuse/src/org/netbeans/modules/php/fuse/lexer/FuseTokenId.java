...
public enum FuseTokenId implements TokenId {
    ...
    private static class PHPLanguageHierarchy extends LanguageHierarchy<FuseTokenId> {
        ...
        protected Lexer<FuseTokenId> createLexer(LexerRestartInfo<FuseTokenId> info) {
            return FuseLexer.create(info, inPHP);
        }
        ...
        @Override
        protected LanguageEmbedding<?> embedding(Token<FuseTokenId> token,
                LanguagePath languagePath, InputAttributes inputAttributes) {
            FuseTokenId id = token.id();
            if (id == T_INLINE_HTML) {
                return LanguageEmbedding.create(HTMLTokenId.language(), 0, 0, true);
            }
            else if (id == PHPDOC_COMMENT) {
                throw new UnsupportedOperationException("PHPDOC_COMMENT unsupported yet");
