 ...
 */
public enum TmplElTokenId implements TokenId {
    ...
    TmplElTokenId(String primaryCategory) {
    ...
    private static final Language<TmplElTokenId> language = new LanguageHierarchy<TmplElTokenId>() {

        @Override
        protected Collection<TmplElTokenId> createTokenIds() {
            return EnumSet.allOf(TmplElTokenId.class);
        }

        @Override
        protected Map<String, Collection<TmplElTokenId>> createTokenCategories() {
            return null;
        }

        @Override
        protected Lexer<TmplElTokenId> createLexer(LexerRestartInfo<TmplElTokenId> info) {
            return new TmplElLexer(info);
        }

        @Override
        protected LanguageEmbedding<?> embedding(
                Token<TmplElTokenId> token, LanguagePath languagePath, InputAttributes inputAttributes) {
            switch (token.id()) {
                case HTML:
    ...
    public static Language<TmplElTokenId> language() {
        return language;
    }
    ...
    private static synchronized Language<? extends TokenId> getELLanguage() {
        ...
        if (EL_LANGUAGE == null) {
            EL_LANGUAGE = Language.find("text/fuse-el"); //NOI18N
        }
        return EL_LANGUAGE;
    }
}
