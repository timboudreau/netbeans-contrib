...
public class JndiDataType extends NewType
{
		   ...
		   DialogDescriptor.OK_CANCEL_OPTION,
			   ...
			   public void actionPerformed(ActionEvent event)
			   {
			       if (event.getSource()==DialogDescriptor.OK_OPTION)
			       {
			           try
			           {
			               ((JndiRootNode)node).addContext(((NewJndiRootPanel)panel).getLabel(),
						        ...
						        ((NewJndiRootPanel)panel).getCredentials(),((NewJndiRootPanel)panel).getAditionalProperties());
	    ...
	    descriptor= new DialogDescriptor(panel,"New Jndi Directory",
					     ...
					     new ActionListener()
						         ...
						         if (event.getSource()==DialogDescriptor.OK_OPTION)
							 {
							     ...
							     try
							     {
							         context.createSubcontext(cld.getOffset().add(panel.getName()));
							         cld.prepareKeys();
							     }catch(NamingException ne)
							     {
							         TopManager.getDefault().notifyException(ne);
    ...
    {
       if (this.node instanceof JndiRootNode) return "Context";
       else if (this.node instanceof JndiNode) return "Directory";
       else return ""; 
    }
}
