...
class CheckinTable implements AncestorListener, TableModelListener {
    ...
    public static String [] CHECKIN_COLUMNS = new String [] {
                                            CheckinTableModel.COLUMN_NAME_NAME,
                                            CheckinTableModel.COLUMN_NAME_STATUS,
                                            CheckinTableModel.COLUMN_NAME_ACTION,
                                            CheckinTableModel.COLUMN_NAME_PATH
                                        };

    private CheckinTableModel   tableModel;
    ...
    public CheckinTable(JLabel label, String[] columns, String[] sortByColumns) {
        init(label, columns, null);
        this.sortByColumns = sortByColumns;        
        setSortingStatus();            
    }

    public CheckinTable(JLabel label, String[] columns, TableSorter sorter) {
        init(label, columns, sorter);        
    }
    
    private void init(JLabel label, String[] columns, TableSorter sorter) {
        tableModel = new CheckinTableModel(columns);
        ...
        table.getAccessibleContext().setAccessibleDescription(NbBundle.getMessage(CheckinTable.class, "ACSD_CheckinTable")); // NOI18N        
        setColumns(columns);
    }
...
//            } else if ((fileStatus & FileInformation.STATUS_IN_REPOSITORY) == 0) {
    ...
    private class CommitStringsCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (columns[col] == CheckinTableModel.COLUMN_NAME_NAME) {
                TableSorter sorter = (TableSorter) table.getModel();
                CheckinTableModel model = (CheckinTableModel) sorter.getTableModel();
            ...
            } else if (columns[col] == CheckinTableModel.COLUMN_NAME_PATH) {
