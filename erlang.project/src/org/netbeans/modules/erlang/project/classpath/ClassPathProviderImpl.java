...
public final class ClassPathProviderImpl implements ClassPathProvider, PropertyChangeListener {
    ...
    public ClassPathProviderImpl(RakeProjectHelper helper, PropertyEvaluator evaluator, 
                                 SourceRoots sourceRoots, SourceRoots includeRoots, SourceRoots testSourceRoots) {
        this.helper = helper;
    ...
    private FileObject[] getIncludeSrcDir() {
        return this.includeRoots.getRoots();
    }
...
//    
    ...
    public ClassPath findClassPath(FileObject file, String type) {
        ...
        } else */ if (type.equals(RubyProject.SOURCE)) {
            return getSourcepath(file);
        } else if (type.equals(RubyProject.BOOT)) {
            return getBootClassPath();
        } else if (type.equals(RubyProject.COMPILE)) {
            // Bogus
            return getBootClassPath();
        } else {
            return null;
    ...
    public ClassPath[] getProjectClassPaths(String type) {
        if (RubyProject.BOOT.equals(type)) {
            return new ClassPath[]{getBootClassPath()};
        }
        if (RubyProject.SOURCE.equals(type)) {
    ...
    public String getPropertyName (SourceGroup sg, String type) {
        ...
        for (int i=0; i<path.length; i++) {
            if (root.equals(path[i])) {
                if (RubyProject.COMPILE.equals(type)) {
                    return JAVAC_CLASSPATH;
                }
                else if (RubyProject.EXECUTE.equals(type)) {
