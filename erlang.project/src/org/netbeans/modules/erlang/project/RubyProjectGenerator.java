...
import org.netbeans.modules.erlang.makeproject.spi.support.ProjectGenerator;
...
public class RubyProjectGenerator {
    ...
    public static final String DEFAULT_INCLUDE_NAME = "include.dir";
    ...
    public static RakeProjectHelper createProject(File dir, String name, String mainClass, String manifestFile) throws IOException {
        ...
        RakeProjectHelper h = createProject(dirFO, name, "src", "include", "test", mainClass, manifestFile, false); //NOI18N
        Project p = ProjectManager.getDefault().findProject(dirFO);
        ProjectManager.getDefault().saveProject(p);
        FileObject srcFolder = dirFO.createFolder("src"); // NOI18N
        dirFO.createFolder("include"); // NOI18N
        dirFO.createFolder("test"); // NOI18N
        if ( mainClass != null ) {
            createMainClass( mainClass, srcFolder, "Templates/Erlang/main.erl" );
        }
        createMainClass( "Makefile", dirFO, "Templates/Erlang/Makefile" );
        createMainClass( "Emakefile", dirFO, "Templates/Erlang/Emakefile" );
        dirFO.createFolder("ebin"); // NOI18N
        return h;
    }

    public static RakeProjectHelper createProject(final File dir, final String name,
                                                  final File[] sourceFolders, final File[] includeFolders, final File[] testFolders, final String manifestFile) throws IOException {
        assert sourceFolders != null && includeFolders != null && testFolders != null: "Package roots can't be null";   //NOI18N
        final FileObject dirFO = createProjectDir (dir);
        // this constructor creates only java application type
        final RakeProjectHelper h = createProject(dirFO, name, null, null, null, null, manifestFile, false);
        ...
        ProjectManager.mutex().writeAccess(new Mutex.ExceptionAction<Void>() {
            public Void run() throws Exception {
                ...
                nl = data.getElementsByTagNameNS(RubyProjectType.PROJECT_CONFIGURATION_NAMESPACE,"include-roots");  //NOI18N
                assert nl.getLength() == 1;
                Element includeRoots = (Element) nl.item(0);
                ...
                for (int i=0; i<includeFolders.length; i++) {
                    String propName;
                    if (i == 0) {
                        //Name the first include root include.dir to be compatible with NB 4.0
                        propName = "include.dir";       //NOI18N
                    }
                    else {
                        String name = includeFolders[i].getName();
                        propName = name + ".dir";    //NOI18N
                    }
                    ...
                    String includeReference = refHelper.createForeignFileReference(includeFolders[i], RubyProject.SOURCES_TYPE_RUBY);
                    Element root = doc.createElementNS (RubyProjectType.PROJECT_CONFIGURATION_NAMESPACE,"root");   //NOI18N
                    root.setAttribute ("id",propName);   //NOI18N
                    includeRoots.appendChild(root);
                    props = h.getProperties(RakeProjectHelper.PROJECT_PROPERTIES_PATH);
                    props.put(propName,includeReference);
                    h.putProperties(RakeProjectHelper.PROJECT_PROPERTIES_PATH, props); // #47609
                }                 
    ...
    private static RakeProjectHelper createProject(FileObject dirFO, String name,
                                                  String srcRoot, String includeRoot, String testRoot, String mainClass, String manifestFile, boolean isLibrary) throws IOException {
...
//        data.appendChild(minant);
        ...
        Element includeRoots = doc.createElementNS(RubyProjectType.PROJECT_CONFIGURATION_NAMESPACE,"include-roots");  //NOI18N
        if (srcRoot != null) {
            Element root = doc.createElementNS (RubyProjectType.PROJECT_CONFIGURATION_NAMESPACE,"root");   //NOI18N
            root.setAttribute ("id","include.dir");   //NOI18N
            includeRoots.appendChild(root);
            ep.setProperty("include.dir", includeRoot); // NOI18N
        }
        data.appendChild (includeRoots);
