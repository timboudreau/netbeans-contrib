...
import org.netbeans.modules.erlang.project.ui.customizer.MainClassChooser;
import org.netbeans.modules.erlang.project.ui.customizer.MainClassWarning;
...
public class RubyActionProvider implements ActionProvider {
    ...
    public void invokeAction(final String command, final Lookup context) throws IllegalArgumentException {
        ...
        if (COMMAND_RUN.equals(command) || COMMAND_DEBUG.equals(command)) {
            ...
            if (mainClass != null) {
                ...
                runErlangApplication(fileObject, mainClass, displayName, context, COMMAND_DEBUG.equals(command));
                return;
            }
            ...
            String classPath = project.evaluator().getProperty(RubyProjectProperties.JAVAC_CLASSPATH);
            new RubyExecution(new ExecutionDescriptor(displayName, pwd, gemManager.getRake()).
            ...
            if (RakeSupport.isRakeFile(file)) {
                ...
                rake.runRake(null, file, file.getName(), new RubyFileLocator(context, project), true, COMMAND_DEBUG_SINGLE.equals(command));
                return;
            }
            ...
            String displayName = "Erlang Shell"; // TODO - internationalize
            File pwd = getSourceFolder(); // Or project directory?
    ...
    private void runErlangApplication(FileObject fileObject, String target,
            String displayName, final Lookup context, final boolean debug) {
        String options = project.evaluator().getProperty(RubyProjectProperties.RUN_JVM_ARGS);
        ...
        FileObject[] srcPath = project.getSourceRoots().getRoots();
        FileObject[] testPath = project.getTestSourceRoots().getRoots();
	FileObject binPath = project.getProjectDirectory().getFileObject("ebin");
        StringBuilder sb = new StringBuilder();
	if (binPath != null && binPath.isValid()) {
	    sb.append(" -pa " + FileUtil.toFile(binPath).getAbsolutePath());	
	}
        String codePath = sb.toString();
        if (options != null) {
            options = codePath + " " + options;
        } else {
            options = codePath;
        }
    ...
    private void buildAll(final Lookup context) {
        ...
        String options = project.evaluator().getProperty(RubyProjectProperties.JAVAC_COMPILER_ARG);
        
        if (options != null && options.trim().length() == 0) {
            options = null;
        }
        
        options = "-noshell -s make all -s init stop";
        
        // Set the load path from the source and test folders.
        // Load paths are additive so users can add their own in the
        // options field as well.
        FileObject[] includePath = project.getIncludeRoots().getRoots();
        StringBuilder sb = new StringBuilder();
        if (includePath != null && includePath.length > 0) {
            for (FileObject root : includePath) {
                if (sb.length() > 0) {
                    sb.append(' ');
                }
                sb.append("-i \"");
        ...
        String includePathStr = sb.toString();
        if (options != null) {
            options = includePathStr + " " + options;
        } else {
            options = includePathStr;
        }
        
        File pwd = getProjectFolder();
        
        String displayName = "Make compile"; // TODO - internationalize
        new RubyExecution(new ExecutionDescriptor(displayName, pwd).
                initialArgs(options).
                fileLocator(new RubyFileLocator(context, project)).
                addOutputRecognizer(RubyExecution.RUBY_COMPILER)).
