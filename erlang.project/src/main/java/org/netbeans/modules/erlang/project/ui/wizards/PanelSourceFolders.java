...
public class PanelSourceFolders extends SettingsPanel implements PropertyChangeListener {
    ...
    public PanelSourceFolders (Panel panel) {
        ...
        this.includePanel.addPropertyChangeListener (this);
        this.testsPanel.addPropertyChangeListener(this);
        ((FolderList)this.sourcePanel).setRelatedFolderList((FolderList)this.includePanel);
        ((FolderList)this.sourcePanel).setRelatedFolderList((FolderList)this.testsPanel);
        ((FolderList)this.includePanel).setRelatedFolderList((FolderList)this.sourcePanel);
        ((FolderList)this.includePanel).setRelatedFolderList((FolderList)this.testsPanel);
        ((FolderList)this.testsPanel).setRelatedFolderList((FolderList)this.sourcePanel);
        ((FolderList)this.testsPanel).setRelatedFolderList((FolderList)this.includePanel);
    }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (FolderList.PROP_FILES.equals(evt.getPropertyName())) {
            ...
            if (evt.getSource() == this.sourcePanel) {                
                ...
                ((FolderList)this.includePanel).setLastUsedDir 
                        ((File)evt.getNewValue());
            }
            if (evt.getSource() == this.includePanel) {                
    ...
    void read (WizardDescriptor settings) {
        ...
        ((FolderList)this.includePanel).setProjectFolder(projectLocation);
        ...
        File[] includeRoot = (File[]) settings.getProperty ("includeRoot");          //NOI18N
        assert includeRoot != null : "includeRoot property must be initialized!" ;   //NOI18N
        ((FolderList)this.includePanel).setFiles(includeRoot);
        ...
        if (currentDirectory != null && currentDirectory.isDirectory()) {       
            ...
            ((FolderList)includePanel).setLastUsedDir(currentDirectory);
    ...
    void store (WizardDescriptor settings) {
        ...
        File[] includeRoots = ((FolderList)this.includePanel).getFiles();
        File[] testRoots = ((FolderList)this.testsPanel).getFiles();
        settings.putProperty("sourceRoot",sourceRoots);    //NOI18N
        settings.putProperty("includeRoots",includeRoots);    //NOI18N
    ...
    boolean valid (WizardDescriptor settings) {
        ...
        File[] includeRoots = ((FolderList)this.includePanel).getFiles();
        File[] testRoots = ((FolderList)this.testsPanel).getFiles();
        String result = checkValidity (projectLocation, sourceRoots, includeRoots, testRoots);
        if (result == null) {
    ...
    static String checkValidity (final File projectLocation, final File[] sources, final File[] includes, final File[] tests ) {
        ...
        for (int i=0; i<includes.length;i++) {
            if (!includes[i].isDirectory() || !includes[i].canRead()) {
                return MessageFormat.format(NbBundle.getMessage(PanelSourceFolders.class,"MSG_IllegalIncludes"),
                        new Object[] {includes[i].getAbsolutePath()});
            }
            String iloc = includes[i].getAbsolutePath ();
            if (ploc.equals (iloc) || ploc.startsWith (iloc + File.separatorChar)) {
                return NbBundle.getMessage(PanelSourceFolders.class,"MSG_IllegalProjectFolder");
            }
        }
    ...
    private void initComponents() {
        ...
        includePanel = new FolderList (NbBundle.getMessage(PanelSourceFolders.class,"CTL_IncludeRoots"), NbBundle.getMessage(PanelSourceFolders.class,"MNE_IncludeRoots").charAt(0),NbBundle.getMessage(PanelSourceFolders.class,"AD_IncludeRoots"), NbBundle.getMessage(PanelSourceFolders.class,"CTL_AddIncludeRoot"),
            NbBundle.getMessage(PanelSourceFolders.class,"MNE_AddIncludeFolder").charAt(0), NbBundle.getMessage(PanelSourceFolders.class,"AD_AddIncludeFolder"),NbBundle.getMessage(PanelSourceFolders.class,"MNE_RemoveIncludeFolder").charAt(0), NbBundle.getMessage(PanelSourceFolders.class,"AD_RemoveIncludeFolder"));
        ...
        gridBagConstraints.weighty = 0.45;
        gridBagConstraints.insets = new java.awt.Insets(12, 0, 0, 0);
        add(includePanel, gridBagConstraints);
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.fill = java.awt.GridBagConstraints.BOTH;
        gridBagConstraints.anchor = java.awt.GridBagConstraints.NORTHWEST;
        gridBagConstraints.weightx = 1.0;
