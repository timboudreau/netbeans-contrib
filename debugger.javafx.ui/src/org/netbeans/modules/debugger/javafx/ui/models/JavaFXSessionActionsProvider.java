...
/**
 * Provides popup menu for JavaFX session nodes: suspend options and language selection. 
 *
 * @author Maros Sandor
 */
public class JavaFXSessionActionsProvider implements NodeActionsProviderFilter {

    private HashSet         listeners;
    

    public JavaFXSessionActionsProvider () {
    }
    ...
    public Action [] getActions(NodeActionsProvider original, Object node) throws UnknownTypeException {
        ...
        if (!(node instanceof Session) || !SessionsTableModelFilter.isJavaFXSession((Session) node)) {
    ...
    private String localize(String s) {
        return NbBundle.getBundle(JavaFXSessionActionsProvider.class).getString(s);
    }
    ...
    private class CustomizeSession extends AbstractAction implements Presenter.Popup {
        ...
        private JavaFXDebugger dbg;

        public CustomizeSession(JavaFXDebugger dbg) {
            this.dbg = dbg;
        }
        ...
        public JMenuItem getPopupPresenter() {
            ...
            if (dbg.getSuspend() == JavaFXDebugger.SUSPEND_ALL) 
