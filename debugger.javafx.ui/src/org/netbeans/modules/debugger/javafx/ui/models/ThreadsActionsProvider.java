...
import org.netbeans.modules.debugger.javafx.ui.SourcePath;
...
public class ThreadsActionsProvider implements NodeActionsProvider {
    ...
    private Action MAKE_CURRENT_ACTION = Models.createAction (
        ...
        new Models.ActionPerformer () {
            ...
            public void perform (Object[] nodes) {
                ...
                ((JavaFXThread) nodes [0]).makeCurrent ();
            }
        },
    ...
    private static Action GO_TO_SOURCE_ACTION = Models.createAction (
        ...
        new Models.ActionPerformer () {
            public boolean isEnabled (Object node) {
                if (node instanceof MonitorModel.ThreadWithBordel) node = ((MonitorModel.ThreadWithBordel) node).originalThread;
                return isGoToSourceSupported ((JavaFXThread) node);
            }
            ...
            public void perform (Object[] nodes) {
                ...
                sp.showSource ((JavaFXThread) nodes [0], language);
            }
        },
        Models.MULTISELECTION_TYPE_EXACTLY_ONE
    );
    ...
    private Action SUSPEND_ACTION = Models.createAction (
        ...
        new Models.ActionPerformer () {
            public boolean isEnabled (Object node) {
                if (node instanceof MonitorModel.ThreadWithBordel) node = ((MonitorModel.ThreadWithBordel) node).originalThread;
                if (node instanceof JavaFXThread)
                    return !((JavaFXThread) node).isSuspended ();
                else
                    return true;
            }
            ...
            public void perform (Object[] nodes) {
                ...
                for (i = 0; i < k; i++) {
                    ...
                    if (node instanceof JavaFXThread)
                        ((JavaFXThread) node).suspend ();
                    else
                        ((JavaFXThreadGroup) node).suspend ();
                }
            }
        },
            ...
            public void perform (Object[] nodes) {
                ...
                for (i = 0; i < k; i++) {
                    Object node = (nodes[i] instanceof MonitorModel.ThreadWithBordel) ? 
                            ((MonitorModel.ThreadWithBordel) nodes[i]).originalThread : nodes[i];
                    if (node instanceof JavaFXThread)
                        ((JavaFXThread) node).resume ();
                    else
                        ((JavaFXThreadGroup) node).resume ();
                }
            }
        },
        Models.MULTISELECTION_TYPE_ALL
    ...
    public Action[] getActions (Object node) throws UnknownTypeException {
        if (node == TreeModel.ROOT) 
            return new Action [0];
        if (node instanceof JavaFXThreadGroup) {
            return new Action [] {
                RESUME_ACTION,
                SUSPEND_ACTION,
            };
        ...
        if (node instanceof JavaFXThread) {
            JavaFXThread t = (JavaFXThread) node;
    ...
    private static boolean isGoToSourceSupported (JavaFXThread t) {
        String language = DebuggerManager.getDebuggerManager ().
            getCurrentSession ().getCurrentLanguage ();
        if (!t.isSuspended ())
            return false;
