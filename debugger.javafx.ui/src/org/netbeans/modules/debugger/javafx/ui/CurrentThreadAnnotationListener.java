 ...
 * {@link JavaFXDebugger#PROP_CURRENT_THREAD}
 * property and annotates current line and call stack for
 * {@link org.netbeans.api.debugger.javafx.JavaFXThread}s in NetBeans editor.
 *
 * @author Jan Jancura
 */
public class CurrentThreadAnnotationListener extends DebuggerManagerAdapter {
    ...
    private JavaFXThread                      currentThread;
    private JavaFXDebugger                    currentDebugger;
    ...
    /**
     * Listens JavaFXDebuggerEngineImpl and DebuggerManager.
     */
    public void propertyChange (PropertyChangeEvent e) {
        ...
        if (e.getPropertyName () == JavaFXDebugger.PROP_CURRENT_THREAD) {
            updateCurrentThread ();
            annotate ();
        } else
        if (e.getPropertyName () == JavaFXDebugger.PROP_CURRENT_CALL_STACK_FRAME) {
            updateCurrentThread ();
            annotate ();
        } else
    ...
    private void updateCurrentDebugger () {
        JavaFXDebugger newDebugger = getCurrentDebugger ();
    ...
    private void annotate () {
        ...
        if ( (currentThread == null) ||
             (currentDebugger.getState () != JavaFXDebugger.STATE_STOPPED) ) {
        ...
        for (int i = 0; i < sessions.length; i++) {
            if (sessions[i].lookupFirst(null, JavaFXDebugger.class) == currentDebugger) {
