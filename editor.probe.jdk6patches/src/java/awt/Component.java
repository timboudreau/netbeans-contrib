                                           ...
                                           Serializable
{
    ...
    private static final Map<Component, Throwable> origins = new WeakHashMap<Component, Throwable>();
    public static Throwable findOrigin(Component c) {
        return origins.get(c);
    }
    
    private static final java.util.List<Throwable> failedFocusRequests = Collections.synchronizedList(new LinkedList<Throwable>());
    public static Throwable[] getFailedFocusRequests() {
        return failedFocusRequests.toArray(new Throwable [failedFocusRequests.size()]);
    }
    ...
    protected Component() {
        ...
        String msg = "The origin of " + getClass().getName() + "@" + Integer.toHexString(System.identityHashCode(this));
        Throwable origin = new Throwable(msg);
        origins.put(this, origin);
    }
     ...
     *
     * @see sun.awt.SunGraphics2D#isFlipStrategyPreferred(ComponentPeer)
     */
    ...
    {
        boolean success = _requestFocusHelper(temporary, focusedWindowChangeAllowed, cause);
        if (!success) {
            Throwable stacktrace = new Throwable("Failed request focus stacktrace");
            failedFocusRequests.add(stacktrace);
            if (failedFocusRequests.size() > 100) {
                failedFocusRequests.remove(0);
            }
        }
        return success;
    }
    
    private boolean _requestFocusHelper(boolean temporary,
                                     boolean focusedWindowChangeAllowed,
                                     CausedFocusEvent.Cause cause)
    {
