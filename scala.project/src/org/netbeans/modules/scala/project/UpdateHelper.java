...
public class UpdateHelper {
    ...
    private static final boolean TRANSPARENT_UPDATE = Boolean.getBoolean("scalaproject.transparentUpdate");
    ...
    public boolean isCurrent () {
        return ProjectManager.mutex().readAccess(new Mutex.Action<Boolean>() {
            public Boolean run() {
                synchronized (this) {
                    if (isCurrent == null) {
                        if ((cfg.getConfigurationFragment("data","http://www.netbeans.org/ns/scala-project/0",true) != null)) {
    ...
    private void saveUpdate () throws IOException {
        ...
        this.cfg.removeConfigurationFragment("data","http://www.netbeans.org/ns/scala-project/0",true); //NOI18N
    ...
    private synchronized Element getUpdatedSharedConfigurationData () {
        if (cachedElement == null) {
            Element  oldRoot = this.cfg.getConfigurationFragment("data","http://www.netbeans.org/ns/scala-project/0",true);    //NOI18N
            if (oldRoot != null) {
                ...
                oldRoot = this.cfg.getConfigurationFragment("data","http://www.netbeans.org/ns/scala-project/0.1",true);    //NOI18N
                if (oldRoot != null) {
                    Document doc = oldRoot.getOwnerDocument();
                    Element newRoot = doc.createElementNS (J2SEProjectType.PROJECT_CONFIGURATION_NAMESPACE,"data"); //NOI18N
                    copyDocument (doc, oldRoot, newRoot);
