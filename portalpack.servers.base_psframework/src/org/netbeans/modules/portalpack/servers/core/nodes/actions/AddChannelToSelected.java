 ...
 * @author Satya
 */
public class AddChannelToSelected extends CookieAction {
    ...
    protected void performAction(Node[] nodes) {
        ...
        for(int i=0;i<nodes.length;i++) {
            ...
            ActionUtil.addChannelToSelectedList(cookie.getDn(),cookie.getKey(),chChildrenNode.getContainerName(),handler);
            
            chChildrenNode.setChannelFilterType(ChannelChildrenNode.SELECTED_TYPE);
            
            Node parent = cookie.getParentNode();
            
            if(parent != null && parent instanceof  ChannelHolderNode)
            {
                ((ChannelHolderNode)parent).setDisplayText(chChildrenNode.getChannelFilterType());
            }
            chChildrenNode.updateKeys();
        }
    }
    ...
    public String getName(){
        return org.openide.util.NbBundle.getMessage(AddChannelToSelected.class, "ACT_ADD_TO_SELECTED_CHANNELS");
    }
