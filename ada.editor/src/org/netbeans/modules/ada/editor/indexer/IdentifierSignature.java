...
public class IdentifierSignature {
    ...
    private IdentifierSignature(Identifier identifier, ElementKind kind, String typeName, boolean declaration, Boolean pkgmember) {
    ...
    public static void add(List<Statement> statements, String typeName, boolean isPackage, List<IdentifierSignature> results) {
        for (Statement statement : statements) {
            if (statement instanceof FieldsDeclaration) {
                FieldsDeclaration fieldsDeclaration = (FieldsDeclaration) statement;
                add(fieldsDeclaration, typeName, isPackage, results);
            }
        }
    }
    ...
    private static void add(FieldsDeclaration declaration, String typename, Boolean pkgMember, List<IdentifierSignature> results) {
        ...
        for (SingleFieldDeclaration fldDeclaration : fields) {
            add(fldDeclaration, typename, pkgMember, results);
        }
    }

    private static void add(SingleFieldDeclaration field, String typename, Boolean clsMember, List<IdentifierSignature> results) {
        Variable var = field.getName();
        Identifier id = get(var);

        IdentifierSignature is = id != null ? new IdentifierSignature(id, 
