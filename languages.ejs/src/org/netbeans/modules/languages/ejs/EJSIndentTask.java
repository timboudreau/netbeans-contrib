...
/**
 * Indent task for EJS.
 * The important work is realy done by the HTML and js indenters, but
 * the HTML indenter no longer indents non-tag lines, so the EJS indenter
 ...
 * @author Tor Norbye
 */
public class EJSIndentTask implements IndentTask {

    private Context context;

    EJSIndentTask(Context context) {
    ...
    public static void reindent(BaseDocument doc, int start, int end) throws BadLocationException {
        ...
        TokenSequence<?extends EJSTokenId> ts = th.tokenSequence(EJSTokenId.language());
        if (ts == null) {
            return;
        }
