...
/** This is a shameless copy of the implementation of the famous exception manager.
 *
 * @author Jaroslav Tulach, Jesse Glick
 */
public final class OomeErrorManager extends ErrorManager {
    
    public OomeErrorManager() {
     ...
     * @see "#18141"
     */
    OomeErrorManager(PrintStream pw) {
        this(null, defaultSeverity(), pw);
    }
    
    //Ensure that Dump and Install are loaded (just in case Install loaded
    //with a different classloader and would be reloaded here);  we want
    //everything in memory that might need to be before
    //an OOME occurs
    ...
    static {
        a = Dump.class;
        b = Install.class;
        System.err.println("OOME Replacement Error Manager installed");
    }
    ...
    private OomeErrorManager(String pfx, int sev, PrintStream pw) {
        prefix = pfx;
        minLogSeverity = sev;
        logWriter = pw;
        ...
        //System.err.println("OomeErrorManager[" + pfx + " #" + uniquifier + "]: minLogSeverity=" + sev);
    }
    ...
    static {
        System.setProperty("sun.awt.exception.handler", "org.netbeans.core.OomeErrorManager$AWTHandler"); // NOI18N
    }
    ...
    private PrintStream getLogWriter () {
        ...
        PrintStream pw = null;
        try {
            Method m = TopLogging.class.getDeclaredMethod("getLogOutputStream",null);
            m.setAccessible(true);
            pw = (PrintStream) m.invoke(null, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public final ErrorManager getInstance(String name) {
        ...
        return new OomeErrorManager(pfx, sev, logWriter);
    }
    ...
    private static Throwable extractNestedThrowable(Throwable t) {
        synchronized (OomeErrorManager.class) {
            if (NESTS == null) {
    ...
    private synchronized Annotation[] findAnnotations0(Throwable t, boolean recursively, Set alreadyVisited) {
        ...
        if (t instanceof MissingResourceException) {
            ...
            if (cn != null) {
                l.add (new Ann (EXCEPTION, NbBundle.getMessage (OomeErrorManager.class, "EXC_MissingResourceException_class_name", cn), null, null, null));
            }
            String k = mre.getKey ();
            if (k != null) {
                l.add (new Ann (EXCEPTION, NbBundle.getMessage (OomeErrorManager.class, "EXC_MissingResourceException_key", k), null, null, null));
            }
        ...
        if (t instanceof SAXParseException) {
            ...
            if (pubid != null || sysid != null) {
                ...
                String msg;
                if (col != -1 || line != -1) {
                    msg = NbBundle.getMessage(OomeErrorManager.class, "EXC_sax_parse_col_line", new Object[] {String.valueOf(pubid), String.valueOf(sysid), new Integer(col), new Integer(line)});
                } else {
                    msg = NbBundle.getMessage(OomeErrorManager.class, "EXC_sax_parse", String.valueOf(pubid), String.valueOf(sysid));
                }
                if (l == null) {
                    l = new ArrayList(1);
                } else {
