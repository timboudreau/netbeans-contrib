...
import org.netbeans.modules.languages.execution.api.input.InputProcessor;
import org.netbeans.modules.languages.execution.api.input.InputProcessors;
import org.netbeans.modules.languages.execution.api.input.InputReaderTask;
import org.netbeans.modules.languages.execution.api.input.InputReaders;
...
//        Thread errorThread = null;
        ...
        try {
            ...
            executor.submit(InputReaderTask.newTask(
                    InputReaders.forStream(process.getInputStream(), true),
                    InputProcessors.proxy(
                        InputProcessors.printing(io.getOut(), Charset.defaultCharset(), true),
                        snooper)));
            executor.submit(InputReaderTask.newTask(
                    InputReaders.forStream(process.getErrorStream(), true),
                    InputProcessors.printing(io.getErr(), Charset.defaultCharset(), false)));
            executor.submit(InputReaderTask.newTask(
                    InputReaders.forReader(io.getIn(), Charset.defaultCharset(), true),
